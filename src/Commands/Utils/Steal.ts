...
@Command('steal', {
    description: 'Formats the pack and author of the quoted sticker',
    category: 'utils',
    usage: 'steal [quote_message] | [new_pack_name] | [new_author_name]',
    exp: 10,
    cooldown: 15
})
export default class command extends BaseCommand {
    override execute = async (M: Message, { flags, context }: IArgs): Promise<void> => {
        if (!M.quoted || M.quoted.type !== 'stickerMessage') return void M.reply('Quote a sticker to format')
        const pack = context.split('|')
        if (!pack[1])
            return void M.reply(
                `Give me the new name and author of the sticker, Baka!\nExample: ${this.client.config.prefix}steal | By | ${M.sender.username}`
            )
